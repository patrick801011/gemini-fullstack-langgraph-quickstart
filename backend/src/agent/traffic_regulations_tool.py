# traffic_regulations_tool.py

class TrafficRegulationsRAG:
    def __init__(self, regulations_text):
        self.regulations = self._preprocess_text(regulations_text)
        # In a more advanced version, we would initialize a vectorizer and document matrix here
        # For now, we'll use simple keyword matching

    def _preprocess_text(self, text):
        articles = []
        current_article_title_text = None # Stores the "第 X 條" string for the current article
        current_article_content_lines = []
        ...
        for line in text.split('\n'):
            ...
            is_standalone_article_declaration = False
            if line.startswith("第") and line.endswith("條"):
                if "章" not in line and "節" not in line and "編" not in line: # Avoid chapter/section titles
                    parts = line.split()
                    if len(parts) >= 2 and parts[0] == "第" and parts[-1] == "條":
                        # E.g., "第 1 條" -> ["第", "1", "條"]
                        # E.g., "第 10 條之1" -> ["第", "10", "條之1"] (title is whole line)
                        is_standalone_article_declaration = True
                    elif len(parts) == 1 and line != "第條": # Handles "第1條" (no spaces)
                        # e.g. "第1條", check if line[1:-1] is a valid number/identifier
                        identifier_part = line[1:line.rindex("條")] if "條" in line[1:] else "" # Ensure "條" is not the first char
                        if identifier_part and not identifier_part.isspace(): # Check if identifier is not empty or just spaces
                             is_standalone_article_declaration = True

            if is_standalone_article_declaration:
                # This line is an article title.
                # Save the previous article's content if it exists.
                if current_article_title_text and current_article_content_lines:
                    articles.append({
                        "title": current_article_title_text,
                        "content": "\n".join(current_article_content_lines).strip()
                    })

                current_article_title_text = line # The line itself is the title, e.g., "第 1 條"
                current_article_content_lines = [line] # Start the new article's content with this line

            elif current_article_title_text:
                # This line is content for the currently open article.
                current_article_content_lines.append(line)
            # Else, this line is not an article declaration and no article is open (e.g. chapter title, preamble)
            # These lines are currently ignored unless they fall under an article.
    ...
    def query(self, search_term):
        """
        Searches for relevant articles based on the search_term.
        For now, this is a simple keyword match in the content or title.
        """
        ...
        # Try to match article number directly if the query is like "第X條"
        if search_term.startswith("第") and search_term.endswith("條"):
            for article in self.regulations:
                if article["title"] == search_term:
                    return f"找到法規條文：\n{article['title']}\n{article['content']}"
            return f"找不到符合 '{search_term}' 的法規條文。"

        relevant_articles = []
        for article in self.regulations:
            if search_term_lower in article["content"].lower() or search_term_lower in article["title"].lower():
                relevant_articles.append(article)

        if not relevant_articles:
            return f"找不到與 '{search_term}' 相關的法規條文。"

        # For now, return the content of the first few relevant articles
        # In a real RAG, we'd use a generator model here.
        response_parts = [f"找到 {len(relevant_articles)} 條相關法規："]
        for article in relevant_articles[:3]: # Limit to 3 for brevity
            response_parts.append(f"\n--- {article['title']} ---\n{article['content']}")

        return "\n".join(response_parts)

# The full text of the regulations will be loaded here.
REGULATIONS_TEXT = """
第 一 章 總則
第 1 條
...
汽車駕駛人對於六歲以下或需要特別看護之兒童，單獨留置於車内者，處駕駛人新臺幣三千元罰鍰。
...
第 35 條
汽機車駕駛人，駕駛汽機車經測試檢定有下列情形之一，機車駕駛人處新臺幣一萬五千元以上九萬元以下罰鍰，汽車駕駛人處新臺幣三萬元以上十二萬元以下罰鍰，並均当場移置保管該汽機車及吊扣其駕駛執照一年至二年；附載未滿十二歲兒童或因而肇事致人受傷者，並吊扣其駕駛執照二年至四年；致人重傷或死亡者，吊銷其駕駛執照，並不得再考領：
一、酒精濃度超過規定標準。
...
# Instance for use
rag_tool = TrafficRegulationsRAG(REGULATIONS_TEXT)

# Example usage (for testing purposes, will be removed or refactored later)
if __name__ == "__main__":
    tool = TrafficRegulationsRAG(REGULATIONS_TEXT)
    # Test with an article number
    print(tool.query("第 3 條"))
    print("\n" + "="*20 + "\n")
    # Test with a keyword
    print(tool.query("道路"))
    print("\n" + "="*20 + "\n")
    # Test with a keyword not present
    print(tool.query("外星人"))
    print("\n" + "="*20 + "\n")
    # Test with specific content (now present in full text)
    print(tool.query("臨時停車"))
